...
pywinpty's upstream test suite is not used: its blocking ``pty.read()`` calls
hang intermittently on Windows on ARM. This drives both backends with
non-blocking reads and hard deadlines instead.
"""
import os
...
def exercise(backend_name):
    ...
    # WinPTY can return zero while an asynchronous write is pending.
    pty.write(payload + '\r\n')
...
if __name__ == '__main__':
    if len(sys.argv) == 2:
        ...
        binaries = [
            Path(sys.executable),
            *package_dir.glob('*.pyd'),
            Path(sys.prefix) / 'Library/bin/conpty.dll',
            Path(sys.prefix) / 'Library/bin/OpenConsole.exe',
